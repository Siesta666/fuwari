---
title: "Notes of Modern Astronomy"
...
&1 pc = 3.0657 \times 10^{16} m = 3.2616 ly = 206265 AU \\\\
&1Mpc = 10^6 pc\\\\
...
自转最快的是木星，只需要不到$10h$就可以完成一个周期

自转最慢的是金星，约需要$243$天完成一个周期

太阳占据了太阳系$99.9\%$的质量

太阳系距离银河中心的距离大约是$8.2kpc$
...
**临边昏暗（Limb Darkening）**
	...
	我们看到的光可以近似看成来自于 $\tau = \frac{2}{3}$ 的半径处，在中心区域**光学深度**小，在边缘区域大
...
- 能量平衡：太阳能的来源——p-p核聚变 （Fusion）这是质子-质子。核裂变是Fission![fusion.png](fusion.png)

太阳光谱就像是约等于5500K的黑体

**有关核爆炸爆炸波的研究**：
泰勒发表了一篇对于原子弹爆炸波分析的论文，他使用**量纲分析法**得到了爆炸波半径和一系列参数之间的函数关系![Wave.png](Wave.png)
...
s = \sqrt{N}L
$$
平均来说，以光子需要走过$1cm$的路程才会碰撞产生电子，一个光子需要至少十万年才可以走出太阳的表面
...
太阳系距离银河系中心大约是 $8.3 \mathrm{kpc}$ ，约为26000光年
...
当从高能态跃迁到低能态会发出光，波长为 $21cm$，频率是 $1420 \mathrm{MHz}$
...
激光在壁中来回反射接近300次，将臂长等效增长为$1200km$，使用干涉模式来测量相移，而不是直接测量，对于一个$1064nm$的激光，一个$10^{-18}m$的位移可以产生$6 \times 10^{-12} \mathrm{rad}$的相移。
...
对于不旋转的施瓦西黑洞，**光子环**的半径是$R = 1.5R_S$， **黑洞阴影**的半径是 $R = 2.6R_S$
黑洞自转会导致一部分更加亮
...
主流观点认为，星系团的形成是一种**自下而上的过程（Bottom-up）**，许多的小星系群和暗物质晕形成，然后再进行组装合并得到大的星系团。
...
但是根据观测结果来看，这个假说是**不正确**的：虽然我们可以观测到这一类事件，但是数量太少了，这样计算出来的暗物质的质量和已知数据严重不符。
...
宇宙大爆炸假说认为宇宙源于一场大爆炸，然后生成了一种富含基本粒子的**Primitive Soup**，然后随着时间演化，逐渐降温，基本粒子进行碰撞合成原子核，这也是解释为啥宇宙中氢和氦的含量最高的假说。

#### 汤普孙散射

在早期宇宙（38万年）宇宙是不透明的，光子无法进行自由运动，高温电子不断碰撞光子，导致光子的运动方向不断改变，即**汤普孙散射**，一直等到温度降低至大约$3000K$，宇宙才开始变得透明
...
当宇宙温度下降（约$3000K，0.26eV$），电子动能下降，质子可以使用库仑力约束电子，此时，宇宙中产生了大量的中性原子，没有电子的散射作用，于是光子可以散射出去，此刻宇宙变得透明，也形成了**最后散射截面**和**微波背景辐射**

#### Q：为啥是 0.26eV 而不是 13.6eV:

**A**:重子和光子的数量比例约为 $6 \times 10^{-10}$ ，光子远多于重子，根据麦克斯韦分布，我只需要速度大于阈值的光子就可以让电子逸出。

#### 宇宙是一个Inside-Out Ball

如果我们从地球向宇宙看，相当于在时间尺度上看宇宙的演化，我们最后看得到一个边界，就是**最后散射截面**，在这个时刻宇宙变得透明，在边界之外就是不透明的等离子体，那是宇宙的最初形态。

![](原初宇宙图.jpg)

所有的星系都产生于宇宙微波背景辐射$1/100000$的涨落

### 微波背景辐射

#### 光子密度

现在宇宙微波背景辐射的光子密度约为 $400/cm^3$

#### 强度

现在宇宙微波背景辐射相当于 $T = 2.73K$的黑体辐射，现在的光子红移值相对于最后散射截面的光子是 $z = 1100$

![](强度和频率.png)

这张图就显示了光子强度分布随着宇宙演化的变化，宇宙逐渐膨胀，会导致光子的波长被拉长，即发生**红移**，现在的光子强度变成了微波波段。

光子强度的分布类似于 $T = 2.73 K$ 的普朗克黑体辐射模型，我们只需要一个数据点就可以确定整一个分布曲线，这是因为：对于任何一个温度，特定波长的强度都是确定的。

#### CMB的多普勒效应

如果我们将微波背景辐射的温度图放大，可以发现，在不同的方向温度有细小的差异，就像多普勒效应一样，在我们的前方，波长变短，温度看上去升高了 $3.36mK$，我们身后，波长变长，温度看上去降低了 $3.36mK$.

我们可以使用这个计算出太阳系的相对速度，最后计算出太阳系相对处女座有约 $300km/s$ 的速度，有大约 $500km/s$ 相当于巨引源的速度。

#### 涨落

现在宇宙的所有结构都源自于宇宙微波背景辐射的量子涨落，相对幅度大约是$1/100000$，这是宇宙结构的种子

### 重子声波震荡

大爆炸之后，暗物质的势阱导致了振动，震荡波以声波的形式在等离子体中传播，待光子可以自由移动之后，等离子体消失，声波被冻结了，声波的半径被称为 **声学视界**，大概是$150Mpc（Today）$，这种声波导致了CMB的波纹，我们可以使用傅里叶分解，将CMB信号分解成角频率的功率谱。

#### 功率谱

功率谱在1度附近取到极值，这等于**声学视界**，此处重子震荡最强。
...
大尺度的物质分布：天体向中心运动时，在红移空间中观察到其几何结构会被**压扁（Squash）**

#### 高速运动的星系团

小尺度的物质分布：在星系团内部，物质热运动剧烈，在红移空间中观察到其几何结构会被拉长，类似手指，称为**上帝之指（Fingers of God）**
...
一般物质的状态方程是$0$，对于相对论气体$w = \frac{1}{3}$，对于暗能量我们有：

$$
w = \frac{P}{\rho} = -1
$$

**宇宙常量 $\Lambda$**:原本是爱因斯坦为了描述静止的宇宙而提出的参量，但是由于后续发现宇宙正在膨胀而放弃，但是后来又发现宇宙正在加速膨胀，这和暗能量有关，而描述暗能量的数学形式即宇宙常数$\Lambda$

### 中性氢的莱曼吸收

指的是从$n = 1$能级到$n = 2$能级，对应的波长是$121.6nm$，位于紫外波段。远方的类星体发出波，在$121.6nm$处出现吸收高峰，![](莱曼吸收.png)
...
**宇宙尺度因子**是描述空间中宇宙的大小尺度，规定现在的宇宙尺度是 $a(t_c) = 1$

我们可以计算出红移程度：

$$
\frac{\lambda_c}{\lambda_e} = 1 + z = \frac{a(t_c)}{a(t_e)} = \frac{1}{a(t_e)}
$$
...
$$
H(t) = \frac{1}{a(t)}\frac{\mathrm{d} a(t)}{\mathrm{d}t}
$$
...
即BAO的声学视界，现在大约是$150Mpc$，在形成星系的尺度上，我们可以发现，在声学视界处，频率变得更大，这就是因为BAO的作用：

![](BAO.png)

此处的单位是：$h^{-1} Mpc$ 这是将哈勃常数定为 $100 km/s/Mpc$时的大小。我们一般取$h = 0.7$，对应于$H = 70km/s/Mpc$，那么在此处的$100Mpc$大约是$145Mpc$，接近于$150Mpc$，**即声学视界**